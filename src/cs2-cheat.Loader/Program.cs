namespace Cs2Cheat.Loader;

using Microsoft.Extensions.Logging;
using Cs2Cheat.Core.Memory;
using Cs2Cheat.Core.Features;
using Cs2Cheat.Core.Overlay;
using Cs2Cheat.Core.Config;
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        log.LogInformation("waiting for cs2.exe ...");
        int pid = ProcessFinder.WaitForProcess("cs2", TimeSpan.FromSeconds(120));
        if (pid <= 0)
        {
        ...
        var profiles = new ProfileManager("cs2-cheat");
        ...
        log.LogInformation("attaching overlay to 'Counter-Strike 2'");
        if (!overlay.AttachToWindow("Counter-Strike 2"))
